...
from dreamplace.ops.watermark.watermark import watermark_insert, watermark_extract, watermark_attack
import pdb
...
class PlaceDataCollection(object):
    ...
    def __init__(self, pos, params, placedb, device):
        ...
        with torch.no_grad():
            ...
            self.node2fence_region_map_ori = torch.from_numpy(
                placedb.node2fence_region_map_ori).to(device)
            self.flat_region_boxes_start_ori = torch.from_numpy(
                placedb.flat_region_boxes_start_ori).to(device)
            self.flat_region_boxes_ori = torch.from_numpy(
                placedb.flat_region_boxes_ori).to(device)
            ...
            # Set up row parity for legalization
            if params.watermark_type == "row_parity":
                random.seed(params.seed)
                torch.manual_seed(params.seed)
                self.wm_parity_cells = torch.randperm(placedb.num_movable_nodes, dtype=torch.int)[:params.watermark_num]
                self.wm_parity_keys = torch.tensor([random.randint(0, 1) for _ in range(params.watermark_num)], dtype=torch.int)  # 0 for even row, 1 for odd row
                print("row parity", self.wm_parity_cells, self.wm_parity_keys)
...
class PlaceOpCollection(object):
    ...
    def __init__(self):
        ...
        self.legality_check_op_attack = None
        self.legalize_attack_op = None
        self.legalize_op_attack = None
        self.individual_legalize_op_attack = None
...
class BasicPlace(nn.Module):
    ...
    def __init__(self, params, placedb):
        ...
        ## Add row parity check into legality check
        if params.watermark_type == "row_parity":
            self.op_collections.legality_check_op, self.op_collections.legality_check_op_attack = self.build_legality_check(
                params, placedb, self.data_collections, self.device)
        else:
            self.op_collections.legality_check_op, self.op_collections.legality_check_op_attack = self.build_legality_check(
                params, placedb, self.data_collections, self.device)
        # legalization
        if len(placedb.regions) > 0:
            self.op_collections.legalize_op, self.op_collections.individual_legalize_op, self.op_collections.legalize_attack_op, self.op_collections.individual_legalize_op_attack = self.build_multi_fence_region_legalization(
                params, placedb, self.data_collections, self.device)
            print("multi fence region legalization")
        else:
            if params.watermark_type == "row_parity":
                self.op_collections.legalize_op, self.op_collections.legalize_attack_op = self.build_legalization(
                    params, placedb, self.data_collections, self.device)
            else:
                self.op_collections.legalize_op, self.op_collections.legalize_attack_op = self.build_legalization(
                    params, placedb, self.data_collections, self.device)
            print("single fence region legalization")
        # detailed placement
        ...
        ## Watermark placement
        self.op_collections.watermark_op = self.build_watermark_insert(params, placedb, self.data_collections, self.device)
        self.op_collections.watermark_extract_op = self.build_watermark_extract(params, placedb, self.data_collections, self.device)
        self.op_collections.watermark_attack_op = self.build_watermark_attack(params, placedb, self.data_collections, self.device)
    ...
    def build_legality_check(self, params, placedb, data_collections, device):
        ...
        @param device cpu or cuda
        """
        check_plain_op = legality_check.LegalityCheck(
                ...
                num_movable_nodes=placedb.num_movable_nodes,
                wm_parity_cells=None,
                wm_parity_keys=None)

        check_attack_op = legality_check.LegalityCheck(
                node_size_x=data_collections.node_size_x,
                node_size_y=data_collections.node_size_y,
                flat_region_boxes=data_collections.flat_region_boxes_ori,
                flat_region_boxes_start=data_collections.flat_region_boxes_start_ori,
                node2fence_region_map=data_collections.node2fence_region_map_ori,
                xl=placedb.xl,
                yl=placedb.yl,
                xh=placedb.xh,
                yh=placedb.yh,
        ...
        if params.watermark_type == "row_parity":
            check_parity_op = legality_check.LegalityCheck(
                    node_size_x=data_collections.node_size_x,
                    node_size_y=data_collections.node_size_y,
                    flat_region_boxes=data_collections.flat_region_boxes,
                    flat_region_boxes_start=data_collections.flat_region_boxes_start,
                    ...
                    num_terminals=placedb.num_terminals,
                    num_movable_nodes=placedb.num_movable_nodes,
                    wm_parity_cells=data_collections.wm_parity_cells,
                    wm_parity_keys=data_collections.wm_parity_keys)

        if params.watermark_type == "row_parity":
            return check_parity_op, check_plain_op
        else:
            return check_plain_op, check_attack_op
    ...
    def build_legalization(self, params, placedb, data_collections, device):
        ...
        if params.watermark_type == "row_parity":
            gl_wm = legalize_alg(
                ...
                num_terminal_NIs=placedb.num_terminal_NIs,
                num_filler_nodes=placedb.num_filler_nodes,
                wmParityCells=data_collections.wm_parity_cells,
                wmParityKeys=data_collections.wm_parity_keys)
        ...
        if params.watermark_type == "gnn":
            ml_attack = macro_legalize.MacroLegalize(
                node_size_x=data_collections.node_size_x,
                node_size_y=data_collections.node_size_y,
                node_weights=data_collections.num_pins_in_nodes,
                flat_region_boxes=data_collections.flat_region_boxes_ori,
            ...
            al_attack = abacus_legalize.AbacusLegalize(
                node_size_x=data_collections.node_size_x,
                node_size_y=data_collections.node_size_y,
                node_weights=data_collections.num_pins_in_nodes,
                flat_region_boxes=data_collections.flat_region_boxes_ori,
        ...
        def build_legalization_op(pos):
            ...
            pos2 = gl(pos1, pos1)
            if params.watermark_type == "row_parity":
                legal = self.op_collections.legality_check_op_attack(pos2)
            else:
        ...
        def build_legalization_op_wm(pos):
            logging.info("Start legalization")
            pos1 = ml(pos, pos)
            pos2 = gl_wm(pos1, pos1)
            legal = self.op_collections.legality_check_op(pos2)
            if not legal:
                logging.error("legality check failed in greedy legalization")
                return pos2
        ...
        def build_legalization_op_attack(pos):
            logging.info("Start legalization")
            pos1 = ml_attack(pos, pos)
            pos2 = gl_attack(pos1, pos1)
            legal = self.op_collections.legality_check_op_attack(pos2)
            if not legal:
                logging.error("legality check failed in greedy legalization")
                return pos2
            return al_attack(pos1, pos2)

        if params.watermark_type == "row_parity":
            return build_legalization_op_wm, build_legalization_op # the first one is for watermark legalization, the latter one is plain check within wm cells during attack
        return build_legalization_op, build_legalization_op_attack
    ...
    def build_multi_fence_region_legalization(self, params, placedb, data_collections, device):
        ...
        def build_legalization_op_attack(pos):
            for i in range(len(placedb.regions)+1):
                pos, pos_ml, pos_gl = legal_ops[i][2](pos)
                pos_ml_list.append(pos_ml)
                pos_gl_list.append(pos_gl)
            legal = self.op_collections.legality_check_op_attack(pos)
            if not legal:
                logging.error("legality check failed in greedy legalization")
                return pos
            else:
                ### start abacus legalizer
                for i in range(len(placedb.regions)+1):
                    pos = legal_ops[i][3](pos, pos_ml_list[i], pos_gl_list[i])
            return pos

        def build_individual_legalization_ops_attack(pos, region_id):
            pos = legal_ops[region_id][2](pos)[0]
            return pos

        return build_legalization_op, build_individual_legalization_ops, build_legalization_op_attack, build_individual_legalization_ops_attack
    ...
    def build_fence_region_legalization(self, region_id, params, placedb, data_collections, device):
        ...
        #cur_wm_parity_cells = data_collections.wm_parity_cells[fence_region_mask]
        #cur_wm_parity_keys = data_collections.wm_parity_keys[fence_region_mask]
        # get the overlap of data_collections.wm_parity_cells and cur_fence_idx
        if params.watermark_type == "row_parity":
            cur_wm_parity_cells = torch.zeros(num_movable_nodes, dtype=torch.bool, device=device)
            cur_wm_parity_cells[data_collections.wm_parity_cells.long()] = True
            cur_wm_parity_cells = cur_wm_parity_cells[fence_region_mask]
            cur_wm_parity_cells = torch.nonzero(cur_wm_parity_cells).view(-1)

            cur_wm_parity_keys = torch.zeros(num_movable_nodes, device=device, dtype=torch.int)
            cur_wm_parity_keys = cur_wm_parity_keys - 1
            cur_wm_parity_keys[data_collections.wm_parity_cells.long()] = data_collections.wm_parity_keys
            cur_wm_parity_keys = cur_wm_parity_keys[fence_region_mask]
            cur_wm_parity_keys = cur_wm_parity_keys[cur_wm_parity_cells.long()]
        ### num pins in nodes
        ...
        ### flat region boxes
        flat_region_boxes_ori = torch.tensor([], device=node_size_x.device, dtype=data_collections.flat_region_boxes.dtype)
        ### flat region boxes start
        flat_region_boxes_start_ori = torch.tensor([0], device=node_size_x.device, dtype=data_collections.flat_region_boxes_start.dtype)
        ### node2fence region map: movable + terminal
        node2fence_region_map_ori = torch.zeros(num_movable_nodes_fence_region + num_terminals_fence_region, dtype=data_collections.node2fence_region_map.dtype, device=node_size_x.device).fill_(data_collections.node2fence_region_map.max().item())
        ...
        if params.watermark_type == "row_parity":
            gl_wm = greedy_legalize.GreedyLegalize(
                node_size_x=node_size_x,
                node_size_y=node_size_y,
                node_weights=num_pins_in_nodes,
                flat_region_boxes=flat_region_boxes,
                ...
                num_terminal_NIs=placedb.num_terminal_NIs,
                num_filler_nodes=num_filler_nodes_fence_region,
                wmParityCells=cur_wm_parity_cells,
                wmParityKeys=cur_wm_parity_keys)
        ...
        if params.watermark_type == "gnn":
            ml_attack = macro_legalize.MacroLegalize(
                ...
                node_size_y=node_size_y,
                node_weights=num_pins_in_nodes,
                flat_region_boxes=flat_region_boxes_ori,
                flat_region_boxes_start=flat_region_boxes_start_ori,
                node2fence_region_map=node2fence_region_map_ori,
                xl=placedb.xl,
                yl=placedb.yl,
                xh=placedb.xh,
                yh=placedb.yh,
            ...
            gl_attack = greedy_legalize.GreedyLegalize(
                node_size_x=node_size_x,
                node_size_y=node_size_y,
                node_weights=num_pins_in_nodes,
                flat_region_boxes=flat_region_boxes_ori,
        ...
        def build_greedy_legalization_op_attack(pos):
            ### reconstruct pos for fence region
            pos_total = pos.data.clone()
            pos = pos.view(2, -1)
            pos = torch.cat([pos[:, :num_movable_nodes][:, fence_region_mask], ## movable
        ...
        def build_greedy_legalization_op_wm(pos):
            ### reconstruct pos for fence region
            pos_total = pos.data.clone()
            pos = pos.view(2, -1)
            pos = torch.cat([pos[:, :num_movable_nodes][:, fence_region_mask], ## movable
        ...
        def build_abacus_legalization_op_attack(pos_total, pos_ref, pos):
            result = al_attack(pos_ref, pos)
            ### commit abacus results to pos_total
            pos_total = pos_total.view(2, -1)
            result = result.view(2, -1)
            pos_total[0, :num_movable_nodes].masked_scatter_(fence_region_mask, result[0, :num_movable_nodes_fence_region])
        ...
        if params.watermark_type == "row_parity":
            return build_greedy_legalization_op_wm, build_abacus_legalization_op
        return build_greedy_legalization_op, build_abacus_legalization_op, build_greedy_legalization_op_attack, build_abacus_legalization_op_attack
    ...
    def build_detailed_placement(self, params, placedb, data_collections,
        ...
        # wirelength for position
        def build_detailed_placement_op(pos, seed=0, wm_cells=None, keys=None):
            # set random seed for detailed placement
    ...
    def build_watermark_insert(self, params, placedb, data_collections, device):
        def build_wataermark_op(pos):
            #watermark_insert function
            wm_pos, wm_cell_idx, keys = watermark_insert(pos, params, placedb, data_collections, device)
            return wm_pos, wm_cell_idx, keys
        return build_wataermark_op

    def build_watermark_extract(self, params, placedb, data_collections, device):
        def watermark_extract_op(ori_pos, wm_pos, keys, wm_dist, wm_cell_idx=None, phase="before_attack", attack_ratio=0):
            #watermark_extract function
            extract_flag, extract_dist = watermark_extract(ori_pos, wm_pos, keys, params, placedb, data_collections, device, wm_dist, wm_cell_idx, phase, attack_ratio)
            return extract_flag, extract_dist
        return watermark_extract_op
    
    def build_watermark_attack(self, params, placedb, data_collections, device):
        def watermark_attack_op(wm_pos, ratio, attack_method):
            #watermark_attack function
            attack_pos = watermark_attack(wm_pos, params, attack_method, placedb, data_collections, device, ratio)
            #extract_flag = watermark_extract(ori_pos, attack_pos, keys, params, placedb, data_collections, device, wm_cell_idx)
            return attack_pos
        return watermark_attack_op
    ...
    def plot(self, params, placedb, iteration, pos, watermark_cells=None, ablation=False):
        """
        @brief plot layout
        @param params parameters
        @param placedb placement database
        ...
        self.op_collections.draw_place_op(pos, figname, watermark_cells)
