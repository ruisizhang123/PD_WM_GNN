...
template <typename T>
bool rowParityCheck(const T* x, const T* y, const T site_width,
                        const T row_height, const T scale_factor, const T xl,
                        const T yl, const int num_movable_nodes, const int* wm_parity_cells, const int* wm_parity_keys) {
  // use scale factor to control the precision
  T precision = (scale_factor == 1.0) ? 1e-6 : scale_factor * 0.1;
  bool legal_flag = true;
  int size_of_wm_parity_cells = sizeof(wm_parity_cells) / sizeof(wm_parity_cells[0]);

  // check row and site alignment
  for (int i = 0; i < size_of_wm_parity_cells; ++i) {
    int cell_id = wm_parity_cells[i];
    int key = wm_parity_keys[i];
    ...
    if (row_id%2 != key) {
      dreamplacePrint(
          kERROR,
          "node %d (%g, %g) failed row parity check with key %d and row id %d (%g, %g)\n", cell_id,
          node_xl, node_yl, key, row_id, row_yl, row_yh);
      legal_flag = false;
    }
  }
...
template <typename T>
bool legalityCheckKernelCPUParity(const T* x, const T* y, const T* node_size_x,
                            ...
                            const int* wm_parity_cells,
                            const int* wm_parity_keys) {
  bool legal_flag = true;
  ...
  if (!rowParityCheck(x, y, site_width, row_height, scale_factor, xl, yl,
                          num_movable_nodes, wm_parity_cells, wm_parity_keys)) {
      legal_flag = false;
  }
