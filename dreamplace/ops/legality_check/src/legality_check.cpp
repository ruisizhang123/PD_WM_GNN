...
bool legality_check_forward_parity(
    ...
    at::Tensor wm_parity_cells, at::Tensor wm_parity_keys) {
  CHECK_FLAT_CPU(pos);
  CHECK_EVEN(pos);
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(pos, "legalityCheckKernelCPUParity", [&] {
    legal_flag = legalityCheckKernelCPUParity<scalar_t>(
        ...
        DREAMPLACE_TENSOR_DATA_PTR(wm_parity_cells, int),
        DREAMPLACE_TENSOR_DATA_PTR(wm_parity_keys, int));
  });
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward_parity", &DREAMPLACE_NAMESPACE::legality_check_forward_parity, "Legality check forward");
