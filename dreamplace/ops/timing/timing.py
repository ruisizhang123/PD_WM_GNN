...
class TimingIO(Function):
    ...
    def read(params):
        ...
        #if params.watermark_type == "buffer_wm":
...
class TimingOptFunction(Function):
    ...
    def forward(ctx, timer, pos, net_names, pin_names, flat_netpin,
        ...
        if pos.is_cuda:
            ...
            # convert pin_offset_x to float32
            pin_offset_x = pin_offset_x.astype(np.float32)
            pin_offset_y = pin_offset_y.astype(np.float32)
            
            #
            #net_names = [net_names[i] for i in range(len(net_names)) if b'wm' not in net_names[i]]
