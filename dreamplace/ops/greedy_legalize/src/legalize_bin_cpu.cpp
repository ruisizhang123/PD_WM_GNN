...
void legalizeBinCPU(
        ...
        const int* wm_parity_cells, const int* wm_parity_keys,
                ...
                for (unsigned int bi = 0; search_flag && bi < bin_blanks.at(blank_bin_id).size(); ++bi)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (intersect_blank_width >= width)
                        {
                            ...
                            //Increase cost if row_parity cannot be met
                            if (wm_parity_cells[node_id] == 1)
                            {
                                int row_id = floorDiv(target_yl - yl, row_height);
                                if (row_id %2 != wm_parity_keys[node_id])
                                {
                                    cost += yh-yl; 
                                }
                            }
        ...
        ) 
{
    legalizeBinCPU(
            ...
            wm_parity_cells, wm_parity_keys,
