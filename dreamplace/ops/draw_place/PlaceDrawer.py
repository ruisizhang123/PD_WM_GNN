...
class PlaceDrawer(object):
    ...
    def forward(pos,
        ...
        @param watermark_cells array to identify if cell is watermarked or not
        ...
        if len(node_xl) > num_physical_nodes:  # filler is included
            ...
            for i in range(num_physical_nodes, num_nodes):
                draw_rect(node_xl[i], node_yl[i], node_xh[i], node_yh[i])
        # draw cells
        # use different colors for watermark_cells
        #import pdb; pdb.set_trace()
        for i in range(num_movable_nodes):
            if watermark_cells[i] == 1: #watermarked cells
                ctx.set_source_rgba(0, 1, 0, alpha=0.5)  # Solid color
                ctx.rectangle(node_xl[i], node_yl[i], node_xh[i] - node_xl[i],
                                node_yh[i] -
                                node_yl[i])  # Rectangle(xl, yl, w, h)
                ctx.fill()
                ctx.set_source_rgba(0, 0, 0, alpha=0.8)  # Use different color for wm cells
            else:
