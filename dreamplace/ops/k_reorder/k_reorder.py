...
class KReorderFunction(Function):
    ...
    def forward(pos, node_size_x, node_size_y, flat_region_boxes,
        ...
        # convert long to int
        #num_bins_x, num_bins_y, num_movable_nodes, num_terminal_NIs, num_filler_nodes, K, max_iters = int(num_bins_x), int(num_bins_y), int(num_movable_nodes), int(num_terminal_NIs), int(num_filler_nodes), int(K), int(max_iters)
