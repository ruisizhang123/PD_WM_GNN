...
def place_benchmark(params):
    ...
    params.phase = int(2.0)
    position_dict = {}
    import pdb; pdb.set_trace()
    for x_idx in range(int(placedb.xl), int(placedb.xh), int(placedb.row_height*params.benchmark_stride)):
        for y_idx in range(int(placedb.yl), int(placedb.yh), int(placedb.row_height*params.benchmark_stride)):
            import pdb; pdb.set_trace()
            params.position_x = x_idx
            params.position_y = y_idx 
            placedb = PlaceDB.PlaceDB()
            placedb(params)
            ...
            hpwl = metrics[-2]
            #import pdb; pdb.set_trace()
            position_dict[(x_idx, y_idx)] = hpwl
            #import pdb; pdb.set_trace()
             
    # save position_dict
    import pickle
    path = "%s/%s/plot" % (params.result_dir, params.design_name())
    pickle.dump(position_dict, open(path + '/position_dict.pkl', 'wb'))
...
def place(params):
    ...
    with open(path+"/plot/time_combine.txt", "a") as f:
        f.write("place time: %f\n" % (p_time))

    
    gp_out_file = os.path.join(
        path,
        "%s.gp_gnn_%s.%s" % (params.design_name(), params.abla_gamma, params.solution_file_suffix()))
    #import pdb; pdb.set_trace()
